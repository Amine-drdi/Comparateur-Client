...
const PricingResults: React.FC<PricingResultsProps> = ({ results }) => {
  ...
  const handleViewDetails = (product: PricingResult, index: number) => {
    console.log('🎯 PRODUIT SÉLECTIONNÉ POUR DÉTAILS', product);
    
    // IMPORTANT : Les garanties sont dans product.warranty (SINGULIER) pour Néoliane
    const warranties = product.warranty || product.warranties || [];
    console.log('📋 Garanties trouvées:', warranties.length);
    console.log('🔍 Type de produit:', product.provider);
    ...
    const fullProductData = {
      ...
      // CORRECTION CRITIQUE : garantir que c'est un tableau pour Néoliane
      warranties: product.provider === 'Néoliane' 
        ? (Array.isArray(warranties) ? warranties : Object.values(warranties || {}))
        : [],
      
      // Garanties détaillées pour April
      warrantiesDetailed: product.warrantiesDetailed || null,
    ...
    // AJOUTER LA CLÉ DANS L'URL (TRÈS IMPORTANT)
    urlParams.append('storageKey', storageKey);
    
    const detailsUrl = `/product-details?${urlParams.toString()}`;
    console.log('🌐 URL COMPLÈTE avec storageKey:', detailsUrl);

    // 4. Ouvrir la fenêtre
    const newWindow = window.open(detailsUrl, '_blank', 'width=1200,height=800,scrollbars=yes');
    
    // 5. Nettoyer après 5 minutes (au cas où la page ne serait pas fermée)
    setTimeout(() => {
      localStorage.removeItem(storageKey);
      console.log('🗑️ Nettoyage automatique localStorage clé:', storageKey);
    }, 5 * 60 * 1000);

    // 6. Envoyer aussi via postMessage (backup)
    if (newWindow) {
      setTimeout(() => {
        try {
