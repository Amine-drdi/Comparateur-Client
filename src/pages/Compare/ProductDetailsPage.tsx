...
const schema = yup.object({
  ...
  spouseBirthDate: yup.string().optional(),
  spouseRegime: yup.string().optional(),
  childrenCount: yup.number().default(0).min(0).max(6),
  productType: yup.string().default('sante'),
  coverageLevel: yup
    .mixed<'minimum' | 'moyen' | 'fort' | 'maximum'>()
...
const PricingForm: React.FC = () => {
  ...
  const { register, handleSubmit, formState: { errors }, watch, trigger, reset } = useForm<PricingFormData>({
    resolver: yupResolver(schema as any), // Cast pour éviter l'erreur de type
    defaultValues: {
      productType: 'sante',
      regime: 'SS',
      childrenCount: 0,
